...
class Result
{
    public static int countResponseTimeRegressions(List<int> responseTimes)
    {
        int count = 0;
        int counter = 1;
        int elements = responseTimes.Count();
        double prev_addition = 0;
        ...
        for(int i=1; i<elements; i++){
            ...
            prev_addition += responseTimes[i-1];
            prev_average = prev_addition/counter;
            
            if(prev_average < responseTimes[i]){
                count++;
            }
