...
class Solution {
    static int getMoneySpent(int[] keyboards, int[] drives, int b) {
        int min_k = keyboards.Min();
        int min_d = drives.Min();
        int total = 0;

        if (min_d >= b || min_k >= b) {
            return -1;
        } else {
            foreach (int k_price in keyboards) {
                foreach (int d_price in drives) {
                    int temp = k_price + d_price;
                    if (temp == b) {
                        return b;
                    } else if (temp > total && temp < b) {
                        total = temp;
                    }
                }
            }
